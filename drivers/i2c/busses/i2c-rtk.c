...
#define I2C_M_NO_GUARD_TIME     0x0020
#endif
#include <linux/err.h>
...
static void i2c_rtk_dump_msg(const struct i2c_msg *p_msg)
{
	RTK_DEBUG("%s\n", __func__);
...
static int rtk_i2c_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(pdev->dev.of_node, "i2c-num", &i2c_id)) {
		int alias_id = of_alias_get_id(pdev->dev.of_node, "i2c");
		if (alias_id >= 0)
			i2c_id = alias_id;
		else
			i2c_id = pdev->id < 0 ? 0 : pdev->id;
	}
	...
	strscpy(i2c_dev->adapter.name, "Realtek I2C adapter",
