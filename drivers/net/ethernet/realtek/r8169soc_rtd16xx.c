...
#define reset_control_get(dev, id) reset_control_get_optional_exclusive(dev, id)
#define KOBJ_LINKUP KOBJ_CHANGE
#define KOBJ_LINKDOWN KOBJ_CHANGE
#define synchronize_sched synchronize_rcu
#define RTK_PM_STATE 0

static struct clk *rtk_clk_get_optional(struct device *dev, const char *id)
{
...
static int rtk_clk_prepare_enable(struct clk *clk)
{
	return clk ? clk_prepare_enable(clk) : 0;
}

static void rtk_clk_disable_unprepare(struct clk *clk)
{
	if (clk)
		clk_disable_unprepare(clk);
}

static bool __maybe_unused rtk_clk_is_enabled(struct clk *clk)
{
	return !clk || __clk_is_enabled(clk);
}
...
/* procfs debug hooks use pre-5.6 file_operations APIs. */
#undef RTL_PROC
static u8 wol_enable;
...
//	struct device *device;
	...
	struct led_classdev led_orange;
	spinlock_t led_lock;
	u32 current_ledsel;
			  ...
			  bool high)
{
	...
	if (!strcmp(c->msg, "rtl_csiar_cond"))
		netif_dbg(tp, drv, tp->dev, "%s == %d (loop: %d, delay: %d).\n",
			  c->msg, !high, n, d);
	else
...
static unsigned int __maybe_unused rtl8169_xmii_always_link_ok(void __iomem *ioaddr)
{
	return true;
}
...
static void __maybe_unused r8169_display_eee_info(struct net_device *dev, struct rtl8169_private *tp)
{
					...
					void __iomem *ioaddr, bool pm)
{
	...
	if (tp->link_ok(ioaddr)) {
		...
		if (!was_up && net_ratelimit())
			netif_info(tp, ifup, dev, "Kabel LAN berhasil tersambung (link up) - Hardware siap beroperasi!\n");
	} else {
		netif_carrier_off(dev);
		if (was_up)
			netif_info(tp, ifdown, dev, "Kabel LAN terputus (link down) - Menunggu dicolok kembali...\n");
		if (pm)
...
static int __maybe_unused rtl8169_set_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	int ret;
...
static int __maybe_unused rtl8169_get_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	struct rtl8169_private *tp = netdev_priv(dev);
	int rc;
...
static int rtl_set_mac_address(struct net_device *dev, void *p)
{
	...
	netdev_info(dev, "Log Trace: Fungsi rtl_set_mac_address dipanggil oleh sistem...\n");
	...
	rtl_rar_set(tp, (u8 *)dev->dev_addr);

	netdev_info(dev, "Sukses ganti MAC secara live ke %pM\n", dev->dev_addr);
...
static void rtl_set_rx_mode(struct net_device *dev)
{
	 ...
	 * Always accept all packets to fix bridge hardware MAC filtering issues 
	 * on Realtek RTD1619 GMAC.
	 */
	...
	if (dev->flags & IFF_PROMISC)
		netdev_info(dev, "Mode Promiscuous AKTIF - Port masuk ke dalam mode Bridge / Jaringan Berbagi!\n");
	else
		netdev_info(dev, "Mode Promiscuous MATI - Port berjalan sebagai antarmuka tunggal normal.\n");
...
static void rtl8169_green_led_set(struct led_classdev *led_cdev, enum led_brightness brightness)
{
	struct rtl8169_private *tp = container_of(led_cdev, struct rtl8169_private, led_green);
	void __iomem *ioaddr = tp->mmio_addr;
	unsigned long flags;
	spin_lock_irqsave(&tp->led_lock, flags);
	if (brightness == LED_OFF) {
		tp->current_ledsel &= ~(0xf << 4);
	} else {
		tp->current_ledsel &= ~(0xf << 4);
		tp->current_ledsel |= (0x8 << 4);
	}
	RTL_W32(LEDSEL, tp->current_ledsel);
	spin_unlock_irqrestore(&tp->led_lock, flags);
}

static void rtl8169_orange_led_set(struct led_classdev *led_cdev, enum led_brightness brightness)
{
	struct rtl8169_private *tp = container_of(led_cdev, struct rtl8169_private, led_orange);
	void __iomem *ioaddr = tp->mmio_addr;
	unsigned long flags;
	spin_lock_irqsave(&tp->led_lock, flags);
	if (brightness == LED_OFF) {
		tp->current_ledsel &= ~(0xf << 8);
	} else {
		tp->current_ledsel &= ~(0xf << 8);
		tp->current_ledsel |= (0x8 << 8);
	}
	RTL_W32(LEDSEL, tp->current_ledsel);
	spin_unlock_irqrestore(&tp->led_lock, flags);
}
...
static int rtl8169_change_mtu(struct net_device *dev, int new_mtu)
{
	...
	netdev_info(dev, "Log Trace: Fungsi rtl8169_change_mtu (Mengubah ukuran MTU ke %d)\n", new_mtu);
...
static int rtl8169_close(struct net_device *dev)
{
	...
	netdev_info(dev, "Log Trace: Fungsi rtl8169_close (Port sedang dimatikan)\n");
...
static int rtl_open(struct net_device *dev)
{
	...
	netdev_info(dev, "Log Trace: Fungsi rtl_open (Port sedang dihidupkan)\n");
...
static void rtl_remove_one(struct platform_device *pdev)
{
	...
	led_classdev_unregister(&tp->led_green);
	led_classdev_unregister(&tp->led_orange);
...
{
	u32 tmp;
	struct clk *clk_etn_sys  = rtk_clk_get_optional(&tp->pdev->dev, "etn_sys");
	struct clk *clk_etn_250m = rtk_clk_get_optional(&tp->pdev->dev, "etn_250m");
	...
	rtk_clk_prepare_enable(clk_etn_sys);
	rtk_clk_prepare_enable(clk_etn_250m);

	/* reg_0x9800708c[12:11] = 00 */
	/* ISO spec, clock enable bit for etn clock & etn 250MHz */
	rtk_clk_disable_unprepare(clk_etn_sys);
	rtk_clk_disable_unprepare(clk_etn_250m);
	...
	if (tp->output_mode != OUTPUT_EMBEDDED_PHY) {
		clk_en_sds = rtk_clk_get_optional(&tp->pdev->dev, "sds");
		...
		rtk_clk_prepare_enable(clk_en_sds);

		/* reg_0x9800000c[7] = 0 */
		/* CRT spec, clk_en_sds */
		rtk_clk_disable_unprepare(clk_en_sds);
...
static u32 r8169soc_serdes_init(struct rtl8169_private *tp)
{
	u32 stable_ticks;
	u32 tmp;
	struct clk *clk_en_sds = rtk_clk_get_optional(&tp->pdev->dev, "sds");
	struct reset_control *rstc_sds_reg =
...
static ssize_t ledsel_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t ledsel_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct net_device *ndev = to_net_dev(dev);
	struct rtl8169_private *tp = netdev_priv(ndev);
	void __iomem *ioaddr = tp->mmio_addr;
	u32 val;
	if (kstrtou32(buf, 16, &val) == 0) {
		tp->current_ledsel = val;
		RTL_W32(LEDSEL, val);
	}
	return count;
}
static DEVICE_ATTR_RW(ledsel);
...
rtl_init_one(struct platform_device *pdev)
{
...
#endif
	#if defined(CONFIG_ARCH_RTD129x)
	struct clk *clk_etn_sys  = rtk_clk_get_optional(&pdev->dev, "etn_sys");
	struct clk *clk_etn_250m = rtk_clk_get_optional(&pdev->dev, "etn_250m");
	...
	tp->eee_enable = false; // Forced to false to fix RTD1619 link flapping
	#if defined(CONFIG_ARCH_RTD129x)
...
//#define ETN_DBUS_CLK    0x98007fc0
	...
	#if defined(CONFIG_ARCH_RTD129x)
	if (unlikely(!rtk_clk_is_enabled(clk_etn_sys) || !rtk_clk_is_enabled(clk_etn_250m))) {
...
//	pci_set_master(pdev);
	...
	netif_napi_add_weight(ndev, &tp->napi, rtl8169_poll,
			      R8169_NAPI_WEIGHT);

	/* don't enable SG, IP_CSUM and TSO by default - it might not work
	 * properly for all devices (especially breaks when bridged) */
	...
	netdev_info(ndev, "PATCH: HW Offloads (SG/CSUM/TSO) disabled to fix bridge network crash.\n");
	netdev_info(ndev, "PATCH: IFF_LIVE_ADDR_CHANGE enabled to allow OpenWrt MAC spoofing without errors.\n");
...
//	pci_set_drvdata(pdev, dev);
	...
	spin_lock_init(&tp->led_lock);
	tp->current_ledsel = 0x00060000;
	
	tp->led_green.name = "r8169:green:lan";
	tp->led_green.brightness_set = rtl8169_green_led_set;
	tp->led_green.default_trigger = "netdev";
	led_classdev_register(&pdev->dev, &tp->led_green);

	tp->led_orange.name = "r8169:orange:lan";
	tp->led_orange.brightness_set = rtl8169_orange_led_set;
	tp->led_orange.default_trigger = "netdev";
	led_classdev_register(&pdev->dev, &tp->led_orange);

	device_create_file(&ndev->dev, &dev_attr_ledsel);
