				       ...
				       const struct i2c_device_id **out_id)
{
	...
	while (id->name[0]) {
		for_each_child_of_node(parent, child) {
			ret = of_alias_from_compatible(child, modalias, sizeof(modalias));
			if (ret)
				continue;
				     ...
				     const char *selection_id)
{
	...
	for_each_child_of_node(selector_np, child) {
		...
		ww_mutex_lock(&reg->rdev->mutex, NULL);
		reg->rdev->dev.of_node = child;
		ww_mutex_unlock(&reg->rdev->mutex);
...
static int selector_probe(struct i2c_client *client)
{
	...
	/* probe */
	ret = selection_driver->probe(client);
	if (ret)
		return ret;
