	...
	struct regulator_state *state)
{
	...
	/* if no config */
	if (state->uV == 0 && state->enabled != DISABLE_IN_SUSPEND)
		state->enabled = true;
...
static int sy8827e_regulator_probe(struct i2c_client *client)
{
	...
	if (data->state_suspend.uV == 0 && data->state_suspend.enabled != DISABLE_IN_SUSPEND)
		data->state_suspend.enabled = true;
	if (data->state_coldboot.uV == 0 && data->state_coldboot.enabled != DISABLE_IN_SUSPEND)
