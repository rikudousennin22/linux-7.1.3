...
/* Added to suppress strict warnings in old vendor driver */
...
static int rtkemmc_execute_tuning(struct mmc_host *host, u32 opcode)
{
	...
	case MODE_SDR:
		...
		break;
	case MODE_DDR:
		mmc_Tuning_DDR50(emmc_port, host->ios.timing);
		0;
		break;
	case MODE_HS200:
		mmc_Tuning_HS200(emmc_port,host->ios.timing);
		0;
		break;
	case MODE_HS400:
		mmc_Tuning_HS400(emmc_port,host->ios.timing);		//hs400 still have dqs tuning, so set flag as 1 after dqs tuning
		break;
	default:
		0;        //this flag is to use for sd card check for rcu stall
		break;
	}
...
static void rtkemmc_set_ios(struct mmc_host *host, struct mmc_ios *ios)
{
	...
	if (ios->bus_width == MMC_BUS_WIDTH_8){
		printk(KERN_DEBUG "set bus width 8\n");
		...
		printk(KERN_DEBUG "set bus width 4\n");
...
//	unsigned long flags2;
	...
	if (cur_div == set_div){
		printk(KERN_DEBUG "%s, set_ip_div = cur_div(0x%08x), ignored. \n", DRIVER_NAME, cur_div);
		return;
	}
...
#ifdef EMMC_DEBUG
	//rtk_lockapi_lock2(flags2, _at_("phase"));
	printk(KERN_DEBUG "%s: phase adjust - EMMC_CKGEN_CTL=0x%08x, PLL_EMMC1=%08x, PLL_EMMC2=%08x, PLL_EMMC3=%08x, PLL_EMMC4=%08x\n",
		DRIVER_NAME,
		readl(emmc_port->emmc_membase + EMMC_CKGEN_CTL),
		readl(emmc_port->crt_membase + SYS_PLL_EMMC1),
...
//	unsigned long flags2;
	...
	//rtk_lockapi_lock2(flags2, _at_("rtkemmc_set_freq"));
	printk(KERN_DEBUG "%s: set_freq to 0x%02x, EMMC_CKGEN_CTL=0x%08x, PLL_EMMC1=%08x, PLL_EMMC2=%08x, PLL_EMMC3=%08x, PLL_EMMC4=%08x\n",
		DRIVER_NAME,
...
static void rtkemmc_timeout_timer(struct timer_list *t)
{
	...
	emmc_port = container_of(t, struct rtkemmc_host, timer);
...
void down_speed_handling(struct rtkemmc_host *emmc_port)
{
	...
	if(gCurrentBootMode == MODE_HS200) {
		...
		0;	//do not set the emmc card capability as HS200
		0;	//send cmd 6 again to inform the host SDR50 mode and set the correct bus speed mode
...
//	rtkemmc_writel(1, emmc_port->emmc_membase + EMMC_SWC_SEL);
	...
	if (cmd_info->data)
		...
		make_ip_des((u32)(uintptr_t)data, block_count<<9, emmc_port->desc_paddr, emmc_port);
	else
		BUG_ON(1);
#else
	make_ip_des((u32)(uintptr_t)data, block_count<<9, emmc_port->desc_paddr, emmc_port);
...
static int rtkemmc_probe(struct platform_device *pdev)
{
	...
	if (!emmc_node)
		...
		printk(KERN_DEBUG "%s : emmc of_node found\n",DRIVER_NAME);
	...
	if (of_property_read_u32(pdev->dev.of_node, "hw-semaphore", &addr)) {
		addr = 0x9801a63c;
		pr_debug("EMMC : can't find hw semaphore in dtb, use default - 0x%x\n", addr);
	} else {
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] get speed-step error !!\n",__func__);
	}
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] no driving nf_s0 warning !! \n",__func__);
	}
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] no driving nf_s1 warning !! \n",__func__);
	}
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] no driving nf_s2 warning !! \n",__func__);
	}
	...
	prop = of_get_property(pdev->dev.of_node, "pddrive_nf_s3", &size);
        if (prop) {
                ...
                printk(KERN_DEBUG "[%s] no driving nf_s3 warning !! \n",__func__);
        }
	...
	if (prop) {
		...
		emmc_port->rx_tuning = 1;
		printk(KERN_DEBUG "[%s] no phase_tuning switch node !! \n",__func__);
	}
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] no dqs_tuning switch node !! \n",__func__);
	}
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] no tx & rx reference phase switch node !! \n",__func__);
	}
	...
	if (prop) {
		...
		printk(KERN_DEBUG "[%s] No emmc tuning offset node, using MBR format !! \n",__func__);
	}

	rstc_emmc = devm_reset_control_get(&pdev->dev, NULL);
	if (IS_ERR(rstc_emmc)) {
		printk(KERN_DEBUG "%s: reset_control_get() returns %ld\n", __func__,
			PTR_ERR(rstc_emmc));
		rstc_emmc = NULL;
	}
	clk_en_emmc = devm_clk_get(&pdev->dev, "clk_en_emmc");
	if (IS_ERR(clk_en_emmc)) {
		printk(KERN_DEBUG "%s: clk_get() returns %ld\n", __func__,
			PTR_ERR(clk_en_emmc));
		clk_en_emmc = NULL;
	}
	...
	mmc->remainder = 0;
#endif
	printk(KERN_DEBUG "%s: %s driver initialized\n",
...
static void __exit rtkemmc_remove(struct platform_device *pdev)
{
	...
	if (mmc) {
		...
		timer_delete_sync(&emmc_port->timer);
		iounmap(emmc_port->emmc_membase);
		iounmap(emmc_port->crt_membase);
