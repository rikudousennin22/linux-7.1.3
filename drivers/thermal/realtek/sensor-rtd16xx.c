	...
	int index, int *temp)
{
	...
	if (t < -3000 || t > 150000) {
		...
		/* If still out of range, sensor is not ready yet.
		 * Clamp to a safe value to prevent false critical shutdown. */
		if (t < -3000 || t > 150000) {
			dev_warn(tdev->dev,
				"temp still out of range after reset (%d), clamping to 40000\n", t);
			t = 40000;
		}
	}
	*temp = t;
