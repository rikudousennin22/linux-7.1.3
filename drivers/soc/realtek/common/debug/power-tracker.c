...
static void power_tracker_timer_expired(struct timer_list *t)
{
	struct power_tracker *pt = container_of(t, struct power_tracker, timer);
...
int power_tracker_register(struct power_tracker *pt)
{
	...
	timer_setup(&pt->timer, power_tracker_timer_expired, 0);
