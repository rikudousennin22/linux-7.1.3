...
static irqreturn_t isr_dcsys_dbg(int irq, void *pdev)
{
	struct ec_info ec0;
	struct ec_info ec1;
	u32 int_stat = 0;
...
static void dcsys_dbg_exit(struct platform_device *pdev)
{
	struct device_node *np;
	int sb2_irq;
...
static int dcsys_dbg_suspend(struct device *dev)
{
	pr_info("[%s] Enter\n", DVR_NAME);
...
static int dcsys_dbg_resume(struct device *dev)
{
